...
<Tabs items={['React @/main.tsx', 'Vue @/main.ts']}>
  <Tabs.Tab>

```tsx filename="React @/main.tsx"
...
<Tabs items={['ParentDemo.tsx', 'ChildDemo.tsx']}>
  <Tabs.Tab>

```tsx filename="ParentDemo.tsx"
...
<Tabs items={['@/hooks/useLocalStorage.ts', '@/App.tsx']}>
  <Tabs.Tab>

```ts filename="@/hooks/useLocalStorage.ts"
...
<Tabs items={['@/hooks/useHistory.ts', '@/App.tsx']}>
  <Tabs.Tab>

```ts filename="@/hooks/useHistory.ts"
import { useSyncExternalStore } from "react";
...
<Tabs items={['@/App.tsx', 'vite.config.ts']}>
  <Tabs.Tab>
...
<Tabs items={['错误示例 1', '正确示例 1', '错误示例 2', '正确示例 2']}>
  <Tabs.Tab>
...
export default function App() {
  ...
  return (
    <>
      <button
        className="cursor-pointer rounded-full border-1 p-3"
        onClick={handleClick}
      >
        父组件获取子组件的 DOM 节点
      </button>
...
<Tabs items={['React &lt;Suspense /&gt;', 'Vue &lt;Suspense /&gt;']}>
  <Tabs.Tab>

```tsx filename="React <Suspense />"
...
<Tabs items={['public/data.json', 'ChildAsync.tsx', 'App.tsx']}>
  <Tabs.Tab>
...
<Tabs items={['ChildDemo.tsx', 'App.tsx']}>
  <Tabs.Tab>
...
<Tabs items={['app.module.css', 'App.tsx']}>
  <Tabs.Tab>
...
<Tabs items={['use-styled.ts', 'styled-button.tsx']}>
  <Tabs.Tab>

```ts filename="use-styled.ts"
...
```tsx filename="styled-button.tsx"
